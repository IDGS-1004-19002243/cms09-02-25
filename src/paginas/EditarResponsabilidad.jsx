...
export default function EditarResponsabilidad() {
  const defaultContent = {
    ...
    shambhala: {
      ...
      // bloques manejables desde el editor
  ...
  const openEditor = (section) => {
    ...
    // Prepare form fields; include energia stats when editing that section
    setForm({
  ...
  const saveChanges = () => {
    ...
    alert('✅ Cambios aplicados (no persistidos en localStorage)');
    closeEditor();
  };
  ...
  return (
    <div className="bg-white text-caborca-cafe font-sans">
      <main>
        {/* HERO */}
        <section data-cms-section="hero" className="relative pt-20 bg-gray-50">
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                <h3 className="text-xl font-semibold text-caborca-cafe">Editar: {activeEdit}</h3>
                <button onClick={closeEditor} className="text-2xl text-gray-400 hover:text-gray-600">✕</button>
              </div>
              <div className="p-6 space-y-4">
                {/* Párrafo 2 eliminado según solicitud del usuario */}
                {/* Imagen global eliminada del modal (editores específicos manejan imágenes) */}
                {activeEdit === 'video' && (
                  <div>
